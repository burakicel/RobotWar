...
public class IcelBot2 extends FighterRobot {

	//Variables
	private int health=100;
	private int starter = 0;
	private int numStepsAllowed;
	private int oppDataCounter = 0;
	 ...
	 */
	public IcelBot2(City arena, int a, int s,Direction direction, int id, int attack) {
	...
	private int target(IcelBotOppData[] w) {
		...
		movementList = this.sortMovement(w);
		
		
		
		//Finds the ideal target based on health and distance
		holder = findIdealTarget(healthList,distanceList,movementList);
		//distanceList = this.sortDistance(w);
	 ...
	 * @param movementList = a list for the amount of steps the opponent took ExtendedOppData
	 * @return int = The Id of the potential target
	 */
	private int findIdealTarget(OppData[] healthList, OppData[] distanceList, IcelBotOppData[] movementList) {
		...
		int holder = 0;
			...
			for(int w=0; w<movementList.length; w++){
			...
			for(int i=0; i<healthList.length; i++){
					...
					for(int q=0; q<distanceList.length; q++){
						...
						if (healthList[i].getID() == distanceList[q].getID() && healthList[i].getID() == movementList[w].getID()){
	 ...
	 */
	private IcelBotOppData[] sortMovement(IcelBotOppData[] w) {
		...
		//Selection sort size^2
		for (int q = 0; q<this.movementList.length;q++){

			for (int i = q; i<this.movementList.length;i++){

				if (this.getID() != i && w[i].getHealth()>0){


					//Chooses the smallest value
					if(w[i].getMovementAmount() < movement && i>holder || i==0){
						movement = w[i].getMovementAmount();
						holder = i;
					}
