...
public class TestFighterRobot extends FighterRobot {
	 ...
	 */
	public TestFighterRobot(City arena, int a, int s,Direction direction, int id, int attack) {
	...
	private int target(OppData[] w, int avenue, int street, int j) {
		...
		for (int i = 0; i<w.length;i++){
			...
			if (j != i && w[i].getHealth()>0){
				...
				closeX = Math.abs(w[i].getAvenue()-avenue);
				closeY = Math.abs(w[i].getStreet()-street);

				if(holder<0 || closeX+closeY<avDistance){
					avDistance = closeX+closeY;
					holder = i;
				}
